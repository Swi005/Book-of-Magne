...
\newglossaryentry{AST}{name={\textbf{Abstract Syntax Tree}},description={An Abstract Syntax Tree(AST) is a tree representation of the syntactic structure of our program. 
                                                                Can be represented by using trees or terms, and described bt an algebraic data type or regular tree grammar.}}
...
type VarDecl = (Var,Type)
    ...
    \subsection{Arg Passing}%Difference between args and params
        ...
        \subsubsection{Copy Semantics}
            ...
            In copy semantics all the arguments are passed to the paramters by copying the value into them.
            With copy semantics the procedure arguments are first evaluated and then bound to the parameters when they are added.
            The procedure is then performed, and the results obtained. Those are then copied back to all the arguments that are upd or out.
            To further explain lets look at a trivial example to understand what happens in store.
...
\end{lstlisting}
\newpage
            Now lets look at the main procedure
\begin{lstlisting}
procedure main()
var a : integer;
var b : integer;
...
\end{lstlisting}
    \Cref{fig:mainBeforeCall} shows us how the state looks like before we start executing the procedure call.
    \begin{figure}[h]
        \StateDiagram{97}{97}{U,U,I 42}{b/98,a/99}
        \caption{State before the procedure call}
        \label{fig:mainBeforeCall}
    \end{figure}
    The first thing we do when entering the procedure call is to save the stackfram so we can restore the enviroment to its 
    proper place once we're done. We then clear the enviroment, ensuring that that only variables declared by the procedure is in scope.
    ...
    Now comes the fun part, \textit{cleaning}! Just performing the function isn't enough, if we were to just drop it here we would have an enviroment that 
    is drasticly different then when we started. We could just reset the stackfram to what it was when we started, but then b wouldnt get its new value. 
    ...
    \cref{fig:mainBeforeCall}. As you can see the free pointer now points to where x used to point and b now has the same value as res. That means that the next time we add something to the enviroment it will
    overwrite those values in store.     
